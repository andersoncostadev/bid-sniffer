...
describe('Switch de testes', () => {
  ...
  test('este é o meu terceiro teste', async () => {
    ...
    await fireEvent.changeText(myInput,'Anderson');
    expect(myInput.props.value).toEqual('Anderson');
    //terceiro teste - simulando eventos
  });
  test('este é o meu quarto teste', async () => {
    const {getByPlaceholderText} = render(<Hello/>);
    const myInput = getByPlaceholderText("Informe sua idade")
    await fireEvent.changeText(myInput,'33');
    expect(myInput.props.value).toEqual('33');
  });
